# API 参考 — xense-mcap-viewer

> English: [API.md](API.md)

本文档描述 `xense-mcap-viewer` v1.0.0 的完整公开 API。
...
```ts
type McapViewerPersistedTheme = 'light' | 'dark';  // 'system' 不会持久化
```
...
| 常量 | 值 | 内容 |
|------|-----|------|
| `MCAP_VIEWER_PREFERENCES_STORAGE_KEY` | `'xense.mcap.prefs'` | UI 偏好 JSON |
| `MCAP_VIEWER_LAYOUT_STORAGE_KEY` | `'xense.mcap.layout'` | 面板布局 JSON |
...
将 `FileListItem[]` 转为 `DatasetItem[]`（`McapViewer` 内部使用的格式）。
...
| 类型 | 说明 |
|------|------|
| `McapViewerExtension` | 一个扩展包；可含 `sidebarTabs`、`playbackOverlays`、`timelineOverlays`（与前者同形，排在 `playbackOverlays` 之后合并）。 |
...
| `McapViewerExtensionContext` | 传给扩展渲染器的稳定上下文（含 `playback`、`timeline`、`messages`、`hostContext`）。 |
...
<McapViewer url="/demo.mcap" extensions={[annotationExtension]} />;
```
...
在 [local dev server](http://localhost:5173) 或自托管 SPA 中，地址栏使用单一数据源参数 `url`（以及可选的 UI 参数）。打开或切换数据源会通过 `history.pushState` 更新 `?url=`，刷新后按该参数恢复。
