# Xense MCAP Viewer — Architecture & Design

> Also available in Chinese: [ARCHITECTURE.zh.md](ARCHITECTURE.zh.md)

This document describes the requirements, architecture, and technical design of **Xense MCAP Viewer** — a high-performance, browser-based ROS data visualization application. It covers both the standalone SPA and the embeddable React component delivery forms.
...
Xense MCAP Viewer is a **browser-native** playback and visualization tool for robotics recording files. It runs entirely client-side — no server-side processing required — and supports:
...
- DockView theme synchronized: `dockview-theme-*` + `xense-dockview-theme-*` classes
- Tailwind CSS `darkMode: ['class']` — `dark` class toggled on the root `#xense-mcap-viewer-root` element
...
| Library | Purpose |
|---------|---------|
| Tailwind CSS ^3.4 | Atomic CSS; `important: '#xense-mcap-viewer-root'` prevents style leakage when embedded |
...
└── src/
    ...
    │   ├── viewer/                     # McapViewer / McapViewerProvider / content
