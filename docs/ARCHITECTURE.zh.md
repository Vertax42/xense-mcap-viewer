# Xense MCAP Viewer 需求与架构设计文档

> 本文档为中文版，英文主版请参阅 [ARCHITECTURE.md](ARCHITECTURE.md)。

> 本文档参考 [Foxglove Studio](https://github.com/foxglove/studio) 等开源 ROS 可视化工具与社区实践，整理需求、架构与技术要点，用于指导 Xense MCAP Viewer 的持续演进。
...
- 语言文件放在 `src/shared/intl/{en,zh,ja}.json`（由 `McapViewerProvider` 等加载）

### 2.8 主题

- 支持 Light / Dark / System 三种主题模式
- DockView 主题同步切换：`DockviewReact` 传入 `theme`（`dockview-theme-*` + `xense-dockview-theme-*`），外层 `data-dockview-chrome-theme` 便于测试与宿主诊断
- Tailwind CSS `darkMode: ['class']`，在根容器 `#xense-mcap-viewer-root` 上切换 `dark` class
...
| 库 | 说明 |
|----|------|
| Tailwind CSS ^3.4 | 原子化 CSS，`important: '#xense-mcap-viewer-root'` 避免嵌入时样式泄漏 |
...
| 策略 | Foxglove 做法 | Xense MCAP Viewer 改进方向 |
|------|--------------|-------------------|
...
function CustomPage() {
  return (
    <McapViewerProvider theme="dark" language="zh">
      <MyCustomNavbar />
      {/* 自建布局与面板 */}
      <MyCustomFooter />
    </McapViewerProvider>
...
└── src/
    ...
    │
    ├── app/
    │   └── AppShell.tsx                # Navbar + McapViewerContent 等装配（实现文件见 `src/features/viewer/`）
    │
    ├── core/
    ...
    │   ├── viewer/                     # 对外 `McapViewer`、内部实现与 `McapViewerProvider` / Content
