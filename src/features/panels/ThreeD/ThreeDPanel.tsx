...
import { useMcapViewerTheme } from '@/features/viewer/McapViewerProvider';
...
function warnMeshIssue(meshUrl: string, reason: string): void {
  ...
  console.warn(`Xense MCAP Viewer 3D: skip mesh ${meshUrl}. ${reason}`);
}
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        console.warn(`Xense MCAP Viewer 3D: URDF parse failed. ${message}`);
        onMeshLoadProgressChange?.(null);
      }
    })();
...
const Scene = ({
  ...
  useEffect(() => {
    if (urdfLoadError) {
      console.warn(`Xense MCAP Viewer 3D: ${urdfLoadError}`);
    }
  }, [urdfLoadError]);
...
export const ThreeDPanel: React.FC<ThreeDPanelProps> = ({
  ...
  const { resolvedTheme } = useMcapViewerTheme();
