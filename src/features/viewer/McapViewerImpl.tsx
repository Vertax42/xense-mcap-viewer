...
import { createMcapViewerIntl } from '@/shared/intl/createMcapViewerIntl';
import { McapViewerProvider } from './McapViewerProvider';
...
function propsSignature(props: McapViewerProps): string {
...
function initialUiFromProps(p: McapViewerProps) {
...
export interface McapViewerProps {
  url?: string;
  file?: File;
  urls?: string[];
  files?: File[];
  ...
  /**
   * `'localStorage'`: read/write `xense.mcap.prefs`. `'off'`: no storage (host owns prefs).
...
export const McapViewer: React.FC<McapViewerProps> = (props) => {
  const urlState = props.urlState ?? 'off';
    ...
    void (async () => {
      ...
      for (const t of targets) {
        try {
          ...
          if (!res.ok) {
            console.warn('[McapViewer] dataset list HTTP', res.status, t);
            continue;
          }
          const json: unknown = await res.json();
          merged.push(...parseRemoteDatasetListJson(json));
        } catch (e) {
          console.warn('[McapViewer] dataset list fetch failed', t, e);
        }
      }
  ...
  const offlineIntl = useMemo(() => createMcapViewerIntl(currentLanguage), [currentLanguage]);
  ...
  const recordHistoryEntry = useCallback(
    async (row: Omit<DatasetHistoryStoredEntry, 'id' | 'openedAt'>) => {
      try {
        ...
        console.warn('[McapViewer] dataset history write failed', e);
      }
    ...
    async (
      ...
      try {
        if (isRemoteTar) {
          ...
          setExtraDatasets((prev) => mergeDatasetLists(prev, normalizeMcapViewerSources({ url: resolved })));
  ...
  if (!hasSource) {
    return (
      <McapViewerProvider theme={currentTheme} language={currentLanguage}>
        <div
